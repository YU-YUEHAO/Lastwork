...
contract EthVoting {
    ...
    function doVoting(uint _pId,bytes32 _PHash,bytes memory _sigs) public {
      ...
      //是否已超过提案时限
      if (proposals[_pId].limitTime < currentTime)
        revert("exceed voting time");
      
      //是否已经投过票
      if (proposals[_pId].voters[msg.sender].initialized == true)
       revert("already vote");
      ...
      //记录投票信息
      proposals[_pId].voters[msg.sender] = voter;
      proposals[_pId].voteCount+=1;
    ...
    function queryVoting(uint _pId, address voterAddr) public view returns (uint){
      ...
      //返回投票时间
      return proposals[_pId].voters[voterAddr].voteTimeStamp;
    }	
